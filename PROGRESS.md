...
- Post-review cleanup pass (7 September 2026), fixing 7 of the 9 remaining deferred Minor findings from Tasks 5-7 (see Known Limitations for the 2 left open by design): broadened Go-Back-N's receiver duplicate detection to use `frame_index` instead of only the immediately preceding sequence, and made Stop-and-Wait's receiver re-ACK on out-of-order arrivals like Go-Back-N already does; extended end-to-end ACK-corruption coverage to all three protocols; made `--window` required for Go-Back-N/Selective Repeat and added a stderr warning when Stop-and-Wait's forced window overrides a user-supplied value; removed a dead validation branch and a redundant floor in `sender_main.cpp`; added a stderr warning for a failed `TCP_NODELAY` `setsockopt`; fixed the Makefile's `applications` target to link from the same objects `make all` already builds instead of recompiling three times; and added tests for `generate_report.py`'s previously-untested zero-coincidence and data-derived-comparison branches. Verified with a full clean rebuild and test run (206 PASS, 0 FAIL, up from 194).
...
The following were raised as Minor findings during Task 5-7 code review and the final whole-branch review. A follow-up pass (7 September 2026) fixed all but two, which are recorded here as deliberate, still-open trade-offs rather than presented as if the branch were flawless:

- **Fixed:** the Go-Back-N receiver's duplicate detection now uses `frame_index` (a monotonic per-transfer counter) instead of comparing only against `last_acked_sequence_`, so any older duplicate is recognized, not only the immediately preceding one. Stop-and-Wait's receiver now re-ACKs the last accepted sequence on an out-of-order arrival, matching Go-Back-N. Both changes are metrics/classification precision only -- no downstream metric currently consumes these flags, so the CSV and report are unaffected. New tests: `test_go_back_n`'s "older duplicate detected" and `test_stop_and_wait`'s "out-of-order re-ACKs last delivered".
- **Fixed:** ACK corruption is now exercised end-to-end for all three protocols (previously Selective Repeat only) -- `deterministic ACK corruption {protocol}` in `test_end_to_end.py`.
- **Fixed:** `--window` is now required for Go-Back-N and Selective Repeat (a missing flag fails loudly instead of silently running as Stop-and-Wait); Stop-and-Wait now warns on stderr if a non-1 `--window` is supplied instead of discarding it silently. New CLI-rejection and warning cases in `test_end_to_end.py`.
- **Fixed:** removed the dead validation branch in `parse_unsigned` and the redundant floor in `elapsed_ms_since` (both in `sender_main.cpp`).
- **Fixed:** `disable_carrier_buffering` now warns on stderr if `setsockopt(TCP_NODELAY)` fails, since a silent failure would inflate every RTT/completion metric without explanation.
- **Fixed:** the Makefile's `applications` target now links `sender`/`receiver` from the same `$(BUILD_DIR)/%.o` objects the pattern rule already builds, instead of recompiling all 13 shared sources from `.cpp` a second and third time. Confirmed idempotent (`make all` twice reports nothing to do).
- **Fixed:** added dedicated tests for the previously-untested branches: `render_path_coincidences`' zero-coincidence fallback (all four impairment sweeps forced pairwise distinct), and both branches each of `completion_estimator_caveat`, `gbn_ack_path_retransmission_free`, and `sr_matches_stop_and_wait_retransmissions`.
- **Left open, by design:** the sender still keeps a shadow window-fill copy of state for Go-Back-N metrics (`sender_main.cpp`'s `record_progress`) that duplicates the cumulative-ACK semantics already implemented inside `go_back_n.cpp`. Unifying them would mean exposing more of the ARQ sender's internal window state through its public interface (e.g. "is frame X now acknowledged") purely to serve one metrics computation, which trades a well-isolated, independently-testable protocol module for a wider public surface on a fully-tested, working system. Correct today; worth revisiting only if `go_back_n.cpp`'s cumulative-ACK semantics change.
- **Left open, by design:** `test_end_to_end.py`'s readiness handshake still greps the receiver's human-readable log string (`"listening"`). A wording change to that message would hang the test for 120 seconds instead of failing fast, but the message and the test are maintained together in this same repository, so the coupling is between two files under one person's control rather than a real interface. A more robust handshake (a dedicated control socket or sentinel file) would add real complexity to remove a risk that a `grep` for the message text in the same commit would also catch.

Fixed items were verified together: `make -C C++ clean && make -C C++ all` compiles all 15 translation units with `-Wall -Wextra -Wpedantic -Werror` and no warnings; `make -C C++ test` exits 0 with 206 PASS lines (was 194 before this pass) and 0 FAIL; `git diff --check` and `git status` are both clean.
